        ...
        async.waterfall(tasks, (err) => {
            ...
            state.consumerHandle = Consumer.create({
                ...
                handleMessage: (msg, cb) => {
                    ...
                    handler(VerifiableObject(body), cb);
                }
            });

            // Start/Stop on specific events
            seneca.on('startConsuming', () => {
                console.log("[ X:PubSubPlugin ] Starting consumer for " + pattern.name);
                state.consumerHandle.start() 
            });

            seneca.on('stopConsuming', () => {
                console.log("[ X:PubSubPlugin ] Stopping consumer for " + pattern.name);
                state.consumerHandle.stop();
            });
        })
    }
