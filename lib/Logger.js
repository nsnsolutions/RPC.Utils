...
function Logger(opts) {
    ...
    var header = opts.header || 'SERVICE',
        ...
        _context = new Context(opts.context),
...
Context.prototype.toString = function toString() {
    return jsonic.stringify(this.ctx);
}

Context.prototype.add = function add(name, value) {
    this.ctx[name] = value;
}

Context.prototype.remove = function remove(name) {
    delete this.ctx[name];
}

Context.prototype.clear = function clear() {
    for(let p in this.ctx)
        if(this.ctx.hasOwnProperty(p))
            this.remove(p);
}
