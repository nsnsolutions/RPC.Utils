...
const DummyCacheProvider = require('./DummyCacheProvider');
...
module.exports = function DocumentClient(args) {
    ...
    function initCache() {
        if(opts.disableCache === true)
            return new DummyCacheProvider(opts);

        else
            return opts.cacheProvider || new RedisCacheProvider(opts);
    }
    ...
    function c_flush(params, callback) {
        ...
        var cacheKey = computeCacheKey.record(params.TableName, index, params.Key);
    ...
    function c_get(params, callback) {
        ...
        var cacheKey = computeCacheKey.record(params.TableName, 'PRIMARY', params.Key);
    ...
    function c_getSI(initialParams, callback) {
        ...
        var cacheKey = computeCacheKey.record(params.TableName, params.IndexName, initialParams.Key);
    ...
    function _add2Cache(params, callback) {
        ...
        var tableDescriptionKey = computeCacheKey.table(params.table);
        cache.get(tableDescriptionKey, (err, td) => {

            if(err)
                return callback(err);

            // If there is no TD, we will silently move along.
            else if(!td)
                return callback();
            ...
            var maps = tableDesc2KeyMaps(td),
                ...
                primaryCacheKey = computeCacheKey.record(params.table, primaryKeyMap.index, primaryKey),
                trans = cache.multi();
            ...
            maps.forEach((keyMap) => {
                ...
                var secondaryKey = item2Key(params.item, keyMap.key),
                    secondaryCacheKey = secondaryKey && computeCacheKey.record(params.table, keyMap.index, secondaryKey);
